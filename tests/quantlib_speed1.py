...
# dayCounter = ql.Actual365Fixed()  # ql.DayCounter

# 配置定价引擎
riskFreeCurve = ql.FlatForward(calculation_date, risk_free_rate, dayCounter)
volatilityCurve = ql.BlackConstantVol(calculation_date, ql.TARGET(), volatility, dayCounter)
dividend_curve_handle =  ql.YieldTermStructureHandle(
    ql.FlatForward(calculation_date, 0.02, dayCounter)
    )
