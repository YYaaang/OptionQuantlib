...
def generate_price_path():
    ...
    maturity_date = today + ql.Period(int(maturity * 365), ql.Days)
    calendar = ql.HongKong(ql.HongKong.HKEx)
    endOfMonth = False
    schedule = ql.Schedule(today, maturity_date, ql.Period(steps_type), calendar,
                           ql.ModifiedFollowing, ql.ModifiedFollowing,
                           ql.DateGeneration.Backward, endOfMonth)
...
def calculate_option_prices_along_path(time, prices):
    ...
    # 对路径上的每个点计算期权价格
    for i, (current_date, S) in enumerate(zip(time, prices)):
        # 设置当前日期
